...
<xsl:stylesheet version="1.0"
  ...
  <!-- template names ending in "Internal" duplicate keyrefs -->
  ...
  <xsl:template name="processing_document">
    ...
    <xsl:param name="uri"/>
    <xsl:apply-templates/>
    <xsl:element name="ProcessingDocument">
      ...
      <xsl:if test="$CheckSemantic">
        <xsl:element name="CheckSemantic">
          ...
          <xsl:call-template name="CheckThisDocumentConstraints"/>
        </xsl:element>
      </xsl:if>
      <xsl:if test="$CheckLinkedDocuments and
                    ./t:ExternalReferences/t:ExternalQIFReferences">
        <xsl:element name="CheckLinkedDocuments"/>
        <xsl:call-template name="processing_linked_documents">
          <xsl:with-param name="caller" select="."/>
        </xsl:call-template>
        <!--
The processing_linked_documents immediately above checks the relationships
between the current document and any other documents referenced by the
current document.

The code below first finds all documents in the directed graph of linked
documents headed by the top-level document and then treats each one as the
current document, including checking relationships between it and any other
QIF documents it references.
        -->
        <xsl:if test="($level=0) and
                      ./t:ExternalReferences/t:ExternalQIFReferences">
          <xsl:variable name="ExtQIFRefs"
                      select="./t:ExternalReferences/t:ExternalQIFReferences"/>
          <xsl:variable name="extDocURIlist">
            <xsl:call-template name="extDocURIlister">
              <xsl:with-param name="level" select="0"/>
              <xsl:with-param name="index" select="1"/>
              <xsl:with-param name="ExtQIFRefs" select="$ExtQIFRefs"/>
              <xsl:with-param name="ExtQIFRefsSize"
                           select="count($ExtQIFRefs/t:ExternalQIFDocument)"/>
            </xsl:call-template>
          </xsl:variable>
          <xsl:variable name="extDocURIlistClean"
                        select="normalize-space($extDocURIlist)"/>
          <xsl:variable name="extDocURIlistShort">
            <xsl:call-template name="unduplicate">
              <xsl:with-param name="newString"/>
              <xsl:with-param name="oldString"
                              select="concat($extDocURIlistClean, ' ')"/>
            </xsl:call-template>
          </xsl:variable>
          <xsl:variable name="extDocURIlistBetter"
                select="concat(normalize-space($extDocURIlistShort), ' ')"/>
          <xsl:call-template name="check_linked_documents">
            <xsl:with-param name="documentURIs" select="$extDocURIlistBetter"/>
          </xsl:call-template>
        </xsl:if>
      </xsl:if>
    </xsl:element>
  ...
  <xsl:template name="check_linked_documents">
    <xsl:param name="documentURIs"/>
    <xsl:variable name="firstURI"
                  select="substring-before($documentURIs, ' ')"/>
    <xsl:variable name="restURIs"
                  select="substring-after($documentURIs, ' ')"/>
    <xsl:if test="$firstURI">
      <xsl:for-each
          select="document(concat('../data/', $firstURI))/t:QIFDocument">
        <xsl:call-template name="processing_document">
          <xsl:with-param name="level" select="1"/>
          <xsl:with-param name="uri" select="$firstURI"/>
        </xsl:call-template>
      </xsl:for-each>
      <xsl:call-template name="check_linked_documents">
        <xsl:with-param name="documentURIs" select="$restURIs"/>
      </xsl:call-template>
    </xsl:if>
  </xsl:template>

<!--
The extDocURIlister finds the uris of all the QIFDocument files in a tree
(actually a directed graph with a single root) of QIFDocument
files connected by ExternalQIFReferences. If any QIFDocument is
referenced by more than one other QIFDocument, its uri will appear
multiple time in the list.

The template works by double recursion. It recurses both on QIFDocument
files it references and on siblings (other QIFDocuments referenced by the
same parent).

A level check prevents infinite recursion in the case of circular references.
No warning message is issued.
-->

  <xsl:template name="extDocURIlister">
    <xsl:param name="level"/>
    <xsl:param name="index"/>
    <xsl:param name="ExtQIFRefs"/>
    <xsl:param name="ExtQIFRefsSize"/>

    <xsl:variable name="thisKidURI"
      select="$ExtQIFRefs/t:ExternalQIFDocument[position()=$index]/t:URI"/>
    <xsl:variable name="thisKidDoc"
      select="document($thisKidURI)/t:QIFDocument"/>
    <xsl:variable name="thisKidKids"
        select="$thisKidDoc/t:ExternalReferences/t:ExternalQIFReferences"/>
    <xsl:variable name="thisKidKidsURIs">
      <xsl:if test="$thisKidKids and ($level &lt; 6)">
        <xsl:call-template name="extDocURIlister">
          <xsl:with-param name="level" select="$level+1"/>
          <xsl:with-param name="index" select="1"/>
          <xsl:with-param name="ExtQIFRefs"
                          select="$thisKidKids"/>
          <xsl:with-param name="ExtQIFRefsSize"
                          select="count($thisKidKids/t:ExternalQIFDocument)"/>
        </xsl:call-template> 
      </xsl:if>
    </xsl:variable>
  
    <xsl:variable name="nextKidKidsURIs">
      <xsl:if test="$index &lt; $ExtQIFRefsSize">
        <xsl:call-template name="extDocURIlister">
    ...
    <xsl:value-of
      select="concat($thisKidURI,' ',$thisKidKidsURIs,' ',$nextKidKidsURIs)"/>
  </xsl:template>

  <xsl:template name="unduplicate">
    <xsl:param name="newString"/>
    <xsl:param name="oldString"/>
    <xsl:variable name="firstSubstring"
                  select="substring-before($oldString, ' ')"/>
    <xsl:variable name="rest"
                  select="substring-after($oldString, ' ')"/>
    <xsl:choose>
      <xsl:when test="not($firstSubstring)">
      ...
      <xsl:otherwise>
        <xsl:variable name="return">
          <xsl:call-template name="unduplicate">
            <xsl:with-param name="newString">
              <xsl:choose>
                <xsl:when test="contains($rest, $firstSubstring)">
                  <xsl:value-of select="$newString"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of
                      select="concat($newString, ' ', $firstSubstring)"/>
                </xsl:otherwise> 
              </xsl:choose>
            </xsl:with-param>
            <xsl:with-param name="oldString" select="$rest"/>
          </xsl:call-template>
        </xsl:variable>
  ...
  <!-- CheckThisDocumentConstraints (duplicates keyrefs) -->

  <xsl:template name="CheckThisDocumentConstraints">
    <xsl:variable name="extDocs"
                  select="./t:ExternalReferences/t:ExternalQIFReferences"/>
    <xsl:element name="CheckThisDocumentConstraints"/>
    <xsl:call-template name="processing_characteristic_ids">
      <xsl:with-param name="extDocs" select="$extDocs"/>
    </xsl:call-template>
    <xsl:call-template name="processing_feature_ids">
      <xsl:with-param name="extDocs" select="$extDocs"/>
    </xsl:call-template>
    <xsl:call-template name="processing_product_ids">
      <xsl:with-param name="extDocs" select="$extDocs"/>
    </xsl:call-template>
  </xsl:template>

<!-- START PROCESSING INTERNAL CHARACTERISTIC IDS -->  
  <xsl:template name="processing_characteristic_ids">
    <xsl:param name="extDocs"/>
    <xsl:variable name="charDefs"
                  select="./t:Characteristics/t:CharacteristicDefinitions"/>
    <xsl:variable name="charNoms"
                  select="./t:Characteristics/t:CharacteristicNominals"/>
    <xsl:variable name="charItems"
                  select="./t:Characteristics/t:CharacteristicItems"/>
    
  <!-- Check that each CharacteristicMeasurement has exactly
       one CharacteristicItemId. -->

    <xsl:for-each
        select="./t:Results/t:MeasurementResultsSet/t:MeasurementResults
                /t:MeasuredCharacteristics/t:CharacteristicMeasurements">
      <xsl:for-each
          select="./t:*">
        <xsl:if
            test="count(./t:CharacteristicItemId) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicMeasurement with id
              (<xsl:value-of select="./@id"/>)
              does not have exactly one CharacteristicItemId.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      </xsl:for-each>

<!-- START MEASUREMENT TO ITEM INTERNAL ID CHECKS FOR CHAR'TICS -->

      <xsl:for-each
          select="./t:AngleBetweenCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:AngleBetweenCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of an
              AngleBetweenCharacteristicMeasurement does not identify either
              (1) an AngleBetweenCharacteristicItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      </xsl:for-each>
      ...
      <xsl:for-each
          select="./t:AngleCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:AngleCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of an
              AngleCharacteristicMeasurement does not identify either
              (1) an AngleCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:AngleFromCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:AngleFromCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of an
              AngleFromCharacteristicMeasurement does not identify either
              (1) an AngleFromCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:AngularCoordinateCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:AngularCoordinateCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of an
              AngularCoordinateCharacteristicMeasurement does not identify
              either
              (1) an AngularCoordinateCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:AngularityCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:AngularityCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of an
              AngularityCharacteristicMeasurement does not identify either
              (1) an AngularityCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ChordCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:ChordCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              ChordCharacteristicMeasurement does not identify either
              (1) a ChordCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CircularRunoutCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:CircularRunoutCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              CircularRunoutCharacteristicMeasurement does not identify either
              (1) a CircularRunoutCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CircularityCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:CircularityCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              CircularityCharacteristicMeasurement does not identify either
              (1) a CircularityCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CoaxialityCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:CoaxialityCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              CoaxialityCharacteristicMeasurement does not identify either
              (1) a CoaxialityCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ConcentricityCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:ConcentricityCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              ConcentricityCharacteristicMeasurement does not identify either
              (1) a ConcentricityCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ConicalTaperCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:ConicalTaperCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              ConicalTaperCharacteristicMeasurement does not identify either
              (1) a ConicalTaperCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ConicityCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:ConicityCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              ConicityCharacteristicMeasurement does not identify either
              (1) a ConicityCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CurveLengthCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:CurveLengthCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              CurveLengthCharacteristicMeasurement does not identify either
              (1) a CurveLengthCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CylindricityCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:CylindricityCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              CylindricityCharacteristicMeasurement does not identify either
              (1) a CylindricityCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:DepthCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:DepthCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              DepthCharacteristicMeasurement does not identify either
              (1) a DepthCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:DiameterCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:DiameterCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              DiameterCharacteristicMeasurement does not identify either
              (1) a DiameterCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:DistanceBetweenCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:DistanceBetweenCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              DistanceBetweenCharacteristicMeasurement does not identify either
              (1) a DistanceBetweenCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:DistanceFromCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:DistanceFromCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              DistanceFromCharacteristicMeasurement does not identify either
              (1) a DistanceFromCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:EllipticityCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:EllipticityCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of an
              EllipticityCharacteristicMeasurement does not identify either
              (1) an EllipticityCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:FlatTaperCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:FlatTaperCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              FlatTaperCharacteristicMeasurement does not identify either
              (1) a FlatTaperCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:FlatnessCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:FlatnessCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              FlatnessCharacteristicMeasurement does not identify either
              (1) a FlatnessCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:HeightCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:HeightCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              HeightCharacteristicMeasurement does not identify either
              (1) a HeightCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:LengthCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:LengthCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              LengthCharacteristicMeasurement does not identify either
              (1) a LengthCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:LineProfileCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:LineProfileCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              LineProfileCharacteristicMeasurement does not identify either
              (1) a LineProfileCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:LinearCoordinateCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:LinearCoordinateCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              LinearCoordinateCharacteristicMeasurement does not identify either
              (1) a LinearCoordinateCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="/t:OtherFormCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:OtherFormCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of an
              OtherFormCharacteristicMeasurement does not identify either
              (1) an OtherFormCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ParallelismCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:ParallelismCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              ParallelismCharacteristicMeasurement does not identify either
              (1) a ParallelismCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PerpendicularityCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:PerpendicularityCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              PerpendicularityCharacteristicMeasurement does not identify either
              (1) a PerpendicularityCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PointProfileCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:PointProfileCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              PointProfileCharacteristicMeasurement does not identify either
              (1) a PointProfileCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PositionCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:PositionCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              PositionCharacteristicMeasurement does not identify either
              (1) a PositionCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:RadiusCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:RadiusCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              RadiusCharacteristicMeasurement does not identify either
              (1) a RadiusCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SphericalDiameterCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:SphericalDiameterCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              SphericalDiameterCharacteristicMeasurement does not identify
              either
              (1) a SphericalDiameterCharacteristicItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SphericalRadiusCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:SphericalRadiusCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              SphericalRadiusCharacteristicMeasurement does not identify either
              (1) a SphericalRadiusCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SphericityCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:SphericityCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              SphericityCharacteristicMeasurement does not identify either
              (1) a SphericityCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SquareCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:SquareCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              SquareCharacteristicMeasurement does not identify either
              (1) a SquareCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:StraightnessCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:StraightnessCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              StraightnessCharacteristicMeasurement does not identify either
              (1) a StraightnessCharacteristicItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SurfaceProfileCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:SurfaceProfileCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              SurfaceProfileCharacteristicMeasurement does not identify either
              (1) a SurfaceProfileCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SurfaceProfileNonUniformCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:SurfaceProfileNonUniformCharacteristicItem[@id = $itemId])
                  + count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              SurfaceProfileNonUniformCharacteristicMeasurement does not
              identify either (1) a SurfaceProfileNonUniformCharacteristicItem
              in the document or (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      </xsl:for-each>
      ...
      <xsl:for-each
          select="./t:SurfaceTextureCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:SurfaceTextureCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              SurfaceTextureCharacteristicMeasurement does not identify either
              (1) a SurfaceTextureCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SymmetryCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:SymmetryCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              SymmetryCharacteristicMeasurement does not identify either
              (1) a SymmetryCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ThicknessCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:ThicknessCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              ThicknessCharacteristicMeasurement does not identify either
              (1) a ThicknessCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ThreadCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:ThreadCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId]))
                  != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              ThreadCharacteristicMeasurement does not identify either
              (1) a ThreadCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ToroidicityCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:ToroidicityCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId]))
                  != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              ToroidicityCharacteristicMeasurement does not identify either
              (1) a ToroidicityCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:TotalRunoutCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:TotalRunoutCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              TotalRunoutCharacteristicMeasurement does not identify either
              (1) a TotalRunoutCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:UserDefinedAngularCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:UserDefinedAngularCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              UserDefinedAngularCharacteristicMeasurement does not identify
              either
              (1) a UserDefinedAngularCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:UserDefinedAreaCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:UserDefinedAreaCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              UserDefinedAreaCharacteristicMeasurement does not identify either
              (1) a UserDefinedAreaCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:UserDefinedAttributeCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:UserDefinedAttributeCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              UserDefinedAttributeCharacteristicMeasurement does not identify
              either (1) a UserDefinedAttributeCharacteristicItem in the
              document or (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      </xsl:for-each>
      ...
      <xsl:for-each
          select="./t:UserDefinedForceCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:UserDefinedForceCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              UserDefinedForceCharacteristicMeasurement does not identify either
              (1) a UserDefinedForceCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:UserDefinedLinearCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:UserDefinedLinearCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              UserDefinedLinearCharacteristicMeasurement does not identify either
              (1) a UserDefinedLinearCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:UserDefinedMassCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:UserDefinedMassCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              UserDefinedMassCharacteristicMeasurement does not identify either
              (1) a UserDefinedMassCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:UserDefinedPressureCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:UserDefinedPressureCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              UserDefinedPressureCharacteristicMeasurement does not identify
              either (1) a UserDefinedPressureCharacteristicItem in the
              document or (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:UserDefinedSpeedCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:UserDefinedSpeedCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              UserDefinedSpeedCharacteristicMeasurement does not identify either
              (1) a UserDefinedSpeedCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:UserDefinedTemperatureCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:UserDefinedTemperatureCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              UserDefinedTemperatureCharacteristicMeasurement does not identify
              either (1) a UserDefinedTemperatureCharacteristicItem in the
              document or (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:UserDefinedTimeCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:UserDefinedTimeCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              UserDefinedTimeCharacteristicMeasurement does not identify either
              (1) a UserDefinedTimeCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:UserDefinedUnitCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:UserDefinedUnitCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              UserDefinedUnitCharacteristicMeasurement does not identify either
              (1) a UserDefinedUnitCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldBevelCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldBevelCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldBevelCharacteristicMeasurement does not identify
              either (1) a WeldBevelCharacteristicItem in the document
              or (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      </xsl:for-each>
      ...
      <xsl:for-each
          select="./t:WeldCompoundCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldCompoundCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldCompoundCharacteristicMeasurement does not identify either
              (1) a WeldCompoundCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldEdgeCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldEdgeCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldEdgeCharacteristicMeasurement does not identify either
              (1) a WeldEdgeCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldFilletCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldFilletCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldFilletCharacteristicMeasurement does not identify either
              (1) a WeldFilletCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldFlareBevelCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldFlareBevelCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldFlareBevelCharacteristicMeasurement does not identify either
              (1) a WeldFlareBevelCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldFlareVCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldFlareVCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldFlareVCharacteristicMeasurement does not identify either
              (1) a WeldFlareVCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldJCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldJCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldJCharacteristicMeasurement does not identify either
              (1) a WeldJCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldPlugCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldPlugCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldPlugCharacteristicMeasurement does not identify either
              (1) a WeldPlugCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldScarfCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldScarfCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldScarfCharacteristicMeasurement does not identify either
              (1) a WeldScarfCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldSeamCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldSeamCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldSeamCharacteristicMeasurement does not identify either
              (1) a WeldSeamCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldSlotCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldSlotCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldSlotCharacteristicMeasurement does not identify either
              (1) a WeldSlotCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="/t:WeldSpotCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldSpotCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldSpotCharacteristicMeasurement does not identify either
              (1) a WeldSpotCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldSquareCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldSquareCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldSquareCharacteristicMeasurement does not identify either
              (1) a WeldSquareCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldStudCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldStudCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldStudCharacteristicMeasurement does not identify either
              (1) a WeldStudCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldSurfacingCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldSurfacingCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldSurfacingCharacteristicMeasurement does not identify either
              (1) a WeldSurfacingCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldUCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldUCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldUCharacteristicMeasurement does not identify either
              (1) a WeldUCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WeldVCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WeldVCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WeldVCharacteristicMeasurement does not identify either
              (1) a WeldVCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:WidthCharacteristicMeasurement
                  /t:CharacteristicItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($charItems
                  /t:WidthCharacteristicItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The CharacteristicItemId (<xsl:value-of select="."/>) of a
              WidthCharacteristicMeasurement does not identify either
              (1) a WidthCharacteristicItem in the document or
              (2) an external QIF document. 
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
...
<!-- END MEASUREMENT TO ITEM INTERNAL ID CHECKS FOR CHARACTERISTICS -->
    
<!-- Check that each CharacteristicItem has
     exactly one CharacteristicNominalId.
-->
    <xsl:for-each
        select="$charItems/t:*">
      <xsl:if
          test="count(./t:CharacteristicNominalId) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicItem with id (<xsl:value-of select="./@id"/>)
            does not have exactly one CharacteristicNominalId.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    </xsl:for-each>
      
<!-- START ITEM TO NOMINAL INTERNAL ID CHECKS FOR CHARACTERISTICS -->
    <xsl:for-each
        select="$charItems/t:AngleBetweenCharacteristicItem">
      <xsl:variable name="nominalId"
                    select="./t:CharacteristicNominalId"/>
      <xsl:if
          test="(count($charNoms
                /t:AngleBetweenCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of an AngleBetweenCharacteristicItem does not identify either
            (1) an AngleBetweenCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:AngleCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of an AngleCharacteristicItem does not identify either
            (1) an AngleCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:AngleFromCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of an AngleFromCharacteristicItem does not identify either
            (1) an AngleFromCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:AngularCoordinateCharacteristicNominal[@id = $nominalId])
                + count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of an AngularCoordinateCharacteristicItem does not identify
            either (1) an AngularCoordinateCharacteristicNominal in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:AngularityCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of an AngularityCharacteristicItem does not identify either
            (1) an AngularityCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:ChordCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a ChordCharacteristicItem does not identify either
            (1) a ChordCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:CircularRunoutCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a CircularRunoutCharacteristicItem does not identify either
            (1) a CircularRunoutCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:CircularityCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a CircularityCharacteristicItem does not identify either
            (1) a CircularityCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:CoaxialityCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a CoaxialityCharacteristicItem does not identify either
            (1) a CoaxialityCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:ConcentricityCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a ConcentricityCharacteristicItem does not identify either
            (1) a ConcentricityCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:ConicalTaperCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a ConicalTaperCharacteristicItem does not identify either
            (1) a ConicalTaperCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:ConicityCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a ConicityCharacteristicItem does not identify either
            (1) a ConicityCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:CurveLengthCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a CurveLengthCharacteristicItem does not identify either
            (1) a CurveLengthCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:CylindricityCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a CylindricityCharacteristicItem does not identify either
            (1) a CylindricityCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:DepthCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a DepthCharacteristicItem does not identify either
            (1) a DepthCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:DiameterCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a DiameterCharacteristicItem does not identify either
            (1) a DiameterCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:DistanceBetweenCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a DistanceBetweenCharacteristicItem does not identify either
            (1) a DistanceBetweenCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:DistanceFromCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a DistanceFromCharacteristicItem does not identify either
            (1) a DistanceFromCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:EllipticityCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of an EllipticityCharacteristicItem does not identify either
            (1) an EllipticityCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:FlatTaperCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a FlatTaperCharacteristicItem does not identify either
            (1) a FlatTaperCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:FlatnessCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a FlatnessCharacteristicItem does not identify either
            (1) a FlatnessCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:HeightCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a HeightCharacteristicItem does not identify either
            (1) a HeightCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:LengthCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a LengthCharacteristicItem does not identify either
            (1) a LengthCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:LineProfileCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a LineProfileCharacteristicItem does not identify either
            (1) a LineProfileCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:LinearCoordinateCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristiceNominalId (<xsl:value-of select="$nominalId"/>)
            of a LinearCoordinateCharacteristicItem does not identify either
            (1) a LinearCoordinateCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:OtherFormCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of an OtherFormCharacteristicItem does not identify either (1) an
            OtherFormCharacteristicNominal in the document or (2) an
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:ParallelismCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a ParallelismCharacteristicItem does not identify either
            (1) a ParallelismCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:PerpendicularityCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a PerpendicularityCharacteristicItem does not identify either
            (1) a PerpendicularityCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:PointProfileCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a PointProfileCharacteristicItem does not identify either (1)
            a PointProfileCharacteristicNominal in the document or (2) an
            external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:PositionCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a PositionCharacteristicItem does not identify either
            (1) a PositionCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:RadiusCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a RadiusCharacteristicItem does not identify either
            (1) a RadiusCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:SphericalDiameterCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a SphericalDiameterCharacteristicItem does not identify either
            (1) a SphericalDiameterCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:SphericalRadiusCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a SphericalRadiusCharacteristicItem does not identify either
            (1) a SphericalRadiusCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:SphericityCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a SphericityCharacteristicItem does not identify either
            (1) a SphericityCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:SquareCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a SquareCharacteristicItem does not identify either
            (1) a SquareCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:StraightnessCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a StraightnessCharacteristicItem does not identify either (1)
            a StraightnessCharacteristicNominal in the document or (2) an
            external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:SurfaceProfileCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a SurfaceProfileCharacteristicItem does not identify either
            (1) a SurfaceProfileCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
            /t:SurfaceProfileNonUniformCharacteristicNominal[@id = $nominalId])
                + count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a SurfaceProfileNonUniformCharacteristicItem does not identify
            either (1) a SurfaceProfileNonUniformCharacteristicNominal
            in the document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:SurfaceTextureCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a SurfaceTextureCharacteristicItem does not identify either
            (1) a SurfaceTextureCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:SymmetryCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a SymmetryCharacteristicItem does not identify either
            (1) a SymmetryCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:ThicknessCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a ThicknessCharacteristicItem does not identify either
            (1) a ThicknessCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:ThreadCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a ThreadCharacteristicItem does not identify either
            (1) a ThreadCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:ToroidicityCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a ToroidicityCharacteristicItem does not identify either
            (1) a ToroidicityCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:TotalRunoutCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a TotalRunoutCharacteristicItem does not identify either
            (1) a TotalRunoutCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:UserDefinedAngularCharacteristicNominal[@id = $nominalId])
                + count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a UserDefinedAngularCharacteristicItem does not identify either
            (1) a UserDefinedAngularCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:UserDefinedAreaCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a UserDefinedAreaCharacteristicItem does not identify either
            (1) a UserDefinedAreaCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:UserDefinedAttributeCharacteristicNominal[@id = $nominalId])
                + count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a UserDefinedAttributeCharacteristicItem does not identify
            either (1) a UserDefinedAttributeCharacteristicNominal in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:UserDefinedForceCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a UserDefinedForceCharacteristicItem does not identify either
            (1) a UserDefinedForceCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:UserDefinedLinearCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a UserDefinedLinearCharacteristicItem does not identify either
            (1) a UserDefinedLinearCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:UserDefinedMassCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a UserDefinedMassCharacteristicItem does not identify either
            (1) a UserDefinedMassCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:UserDefinedPressureCharacteristicNominal[@id = $nominalId])
                + count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a UserDefinedPressureCharacteristicItem does not identify either
            (1) a UserDefinedPressureCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:UserDefinedSpeedCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a UserDefinedSpeedCharacteristicItem does not identify either
            (1) a UserDefinedSpeedCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
             /t:UserDefinedTemperatureCharacteristicNominal[@id = $nominalId])
                + count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a UserDefinedTemperatureCharacteristicItem does not identify
            either (1) a UserDefinedTemperatureCharacteristicNominal in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:UserDefinedTimeCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a UserDefinedTimeCharacteristicItem does not identify either
            (1) a UserDefinedTimeCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:UserDefinedUnitCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a UserDefinedUnitCharacteristicItem does not identify either
            (1) a UserDefinedUnitCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldBevelCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldBevelCharacteristicItem does not identify either
            (1) a WeldBevelCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldCompoundCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldCompoundCharacteristicItem does not identify either
            (1) a WeldCompoundCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldEdgeCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldEdgeCharacteristicItem does not identify either
            (1) a WeldEdgeCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldFilletCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldFilletCharacteristicItem does not identify either
            (1) a WeldFilletCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldFlareBevelCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldFlareBevelCharacteristicItem does not identify either
            (1) a WeldFlareBevelCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldFlareVCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldFlareVCharacteristicItem does not identify either
            (1) a WeldFlareVCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldJCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldJCharacteristicItem does not identify either
            (1) a WeldJCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldPlugCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldPlugCharacteristicItem does not identify either
            (1) a WeldPlugCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldScarfCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldScarfCharacteristicItem does not identify either
            (1) a WeldScarfCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldSeamCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldSeamCharacteristicItem does not identify either
            (1) a WeldSeamCharacteristicNominal in the document or (2) an
            external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldSlotCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldSlotCharacteristicItem does not identify either
            (1) a WeldSlotCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldSpotCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldSpotCharacteristicItem does not identify either
            (1) a WeldSpotCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldSquareCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldSquareCharacteristicItem does not identify either
            (1) a WeldSquareCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldStudCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldStudCharacteristicItem does not identify either
            (1) a WeldStudCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldSurfacingCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldSurfacingCharacteristicItem does not identify either
            (1) a WeldSurfacingCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldUCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldUCharacteristicItem does not identify either
            (1) a WeldUCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WeldVCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WeldVCharacteristicItem does not identify either
            (1) a WeldVCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
      ...
      <xsl:if
          test="(count($charNoms
                /t:WidthCharacteristicNominal[@id = $nominalId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $nominalId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominalId (<xsl:value-of select="$nominalId"/>)
            of a WidthCharacteristicItem does not identify either
            (1) a WidthCharacteristicNominal in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
...
<!-- END ITEM TO NOMINAL INTERNAL ID CHECKS FOR CHARACTERISTICS -->

  <!-- Check that each CharacteristiNominal has
       exactly one CharacteristicDefinitionId.
  -->
  
    <xsl:for-each
        select="./t:Characteristics/t:CharacteristicNominals/t:*">
      <xsl:if
          test="count(./t:CharacteristicDefinitionId) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicNominal with id (<xsl:value-of select="./@id"/>)
            does not have exactly one CharacteristicDefinitionId.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    </xsl:for-each>

<!-- START NOMINAL TO DEFINITION ID INTERNAL CHECKS FOR CHAR'TICS -->

    <xsl:for-each
          select="$charNoms/t:AngleBetweenCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:AngleBetweenCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of an AngleBetweenCharacteristicNominal does not identify either
            (1) an AngleBetweenCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:AngleCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:AngleCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of an AngleCharacteristicNominal does not identify either (1) an
            AngleCharacteristicDefinition in the document or (2) an
            external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:AngleFromCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:AngleFromCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of an AngleFromCharacteristicNominal does not identify either
            (1) an AngleFromCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:AngularCoordinateCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:AngularCoordinateCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of an AngularCoordinateCharacteristicNominal does not identify
            either (1) an AngularCoordinateCharacteristicDefinition in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:AngularityCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:AngularityCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of an AngularityCharacteristicNominal does not identify either
            (1) an AngularityCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:ChordCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:ChordCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a ChordCharacteristicNominal does not identify either
            (1) a ChordCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:CircularRunoutCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:CircularRunoutCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a CircularRunoutCharacteristicNominal does not identify either
            (1) a CircularRunoutCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:CircularityCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:CircularityCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a CircularityCharacteristicNominal does not identify either
            (1) a CircularityCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:CoaxialityCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:CoaxialityCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a CoaxialityCharacteristicNominal does not identify either
            (1) a CoaxialityCharacteristicDefinition in the document or (2) an
            external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:ConcentricityCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:ConcentricityCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a ConcentricityCharacteristicNominal does not identify either
            (1) a ConcentricityCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:ConicalTaperCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:ConicalTaperCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a ConicalTaperCharacteristicNominal does not identify either
            (1) a ConicalTaperCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:ConicityCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:ConicityCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a ConicityCharacteristicNominal does not identify either
            (1) a ConicityCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:CurveLengthCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:CurveLengthCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a CurveLengthCharacteristicNominal does not identify either
            (1) a CurveLengthCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:CylindricityCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:CylindricityCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a CylindricityCharacteristicNominal does not identify either
            (1) a CylindricityCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:DepthCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:DepthCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a DepthCharacteristicNominal does not identify either
            (1) a DepthCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:DiameterCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:DiameterCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a DiameterCharacteristicNominal does not identify either
            (1) a DiameterCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:DistanceBetweenCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:DistanceBetweenCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a DistanceBetweenCharacteristicNominal does not identify either
            (1) a DistanceBetweenCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:DistanceFromCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:DistanceFromCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a DistanceFromCharacteristicNominal does not identify either
            (1) a DistanceFromCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:EllipticityCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:EllipticityCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a EllipticityCharacteristicNominal does not identify either
            (1) a EllipticityCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:FlatTaperCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:FlatTaperCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a FlatTaperCharacteristicNominal does not identify either
            (1) a FlatTaperCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:FlatnessCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:FlatnessCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a FlatnessCharacteristicNominal does not identify either
            (1) a FlatnessCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:HeightCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:HeightCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a HeightCharacteristicNominal does not identify either
            (1) a HeightCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:LengthCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:LengthCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a LengthCharacteristicNominal does not identify either
            (1) a LengthCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:LineProfileCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:LineProfileCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a LineProfileCharacteristicNominal does not identify either
            (1) a LineProfileCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:LinearCoordinateCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:LinearCoordinateCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristiceDefinitionId (<xsl:value-of select="$defnId"/>)
            of a LinearCoordinateCharacteristicNominal does not identify
            either (1) a LinearCoordinateCharacteristicDefinition in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:OtherFormCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:OtherFormCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a OtherFormCharacteristicNominal does not identify either
            (1) a OtherFormCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:ParallelismCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:ParallelismCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a ParallelismCharacteristicNominal does not identify either
            (1) a ParallelismCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:PerpendicularityCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:PerpendicularityCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a PerpendicularityCharacteristicNominal does not identify
            either (1) a PerpendicularityCharacteristicDefinition in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:PointProfileCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:PointProfileCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a PointProfileCharacteristicNominal does not identify either
            (1) a PointProfileCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:PositionCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:PositionCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a PositionCharacteristicNominal does not identify either
            (1) a PositionCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:RadiusCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:RadiusCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a RadiusCharacteristicNominal does not identify either
            (1) a RadiusCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:SphericalDiameterCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:SphericalDiameterCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a SphericalDiameterCharacteristicNominal does not identify
            either (1) a SphericalDiameterCharacteristicDefinition in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:SphericalRadiusCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:SphericalRadiusCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a SphericalRadiusCharacteristicNominal does not identify either
            (1) a SphericalRadiusCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:SphericityCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:SphericityCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a SphericityCharacteristicNominal does not identify either
            (1) a SphericityCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:SquareCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:SquareCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a SquareCharacteristicNominal does not identify either
            (1) a SquareCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:StraightnessCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:StraightnessCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a StraightnessCharacteristicNominal does not identify either
            (1) a StraightnessCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:SurfaceProfileCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:SurfaceProfileCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a SurfaceProfileCharacteristicNominal does not identify either
            (1) a SurfaceProfileCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:SurfaceProfileNonUniformCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
            /t:SurfaceProfileNonUniformCharacteristicDefinition[@id = $defnId])
                + count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a SurfaceProfileNonUniformCharacteristicNominal does not
            identify either
            (1) a SurfaceProfileNonUniformCharacteristicDefinition
            in the document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:SurfaceTextureCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:SurfaceTextureCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a SurfaceTextureCharacteristicNominal does not identify either
            (1) a SurfaceTextureCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:SymmetryCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:SymmetryCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a SymmetryCharacteristicNominal does not identify either
            (1) a SymmetryCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:ThicknessCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:ThicknessCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a ThicknessCharacteristicNominal does not identify either
            (1) a ThicknessCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:ThreadCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:ThreadCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a ThreadCharacteristicNominal does not identify either
            (1) a ThreadCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:ToroidicityCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:ToroidicityCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a ToroidicityCharacteristicNominal does not identify either
            (1) a ToroidicityCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:TotalRunoutCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:TotalRunoutCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a TotalRunoutCharacteristicNominal does not identify either
            (1) a TotalRunoutCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:UserDefinedAngularCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:UserDefinedAngularCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a UserDefinedAngularCharacteristicNominal does not identify
            either (1) a UserDefinedAngularCharacteristicDefinition in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:UserDefinedAreaCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:UserDefinedAreaCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a UserDefinedAreaCharacteristicNominal does not identify either
            (1) a UserDefinedAreaCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:UserDefinedAttributeCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:UserDefinedAttributeCharacteristicDefinition[@id = $defnId])
                + count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a UserDefinedAttributeCharacteristicNominal does not identify
            either (1) a UserDefinedAttributeCharacteristicDefinition in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:UserDefinedForceCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:UserDefinedForceCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a UserDefinedForceCharacteristicNominal does not identify
            either (1) a UserDefinedForceCharacteristicDefinition in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:UserDefinedLinearCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:UserDefinedLinearCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a UserDefinedLinearCharacteristicNominal does not identify
            either (1) a UserDefinedLinearCharacteristicDefinition in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:UserDefinedMassCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:UserDefinedMassCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a UserDefinedMassCharacteristicNominal does not identify either
            (1) a UserDefinedMassCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:UserDefinedPressureCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:UserDefinedPressureCharacteristicDefinition[@id = $defnId])
                + count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a UserDefinedPressureCharacteristicNominal does not identify
            either (1) a UserDefinedPressureCharacteristicDefinition in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:UserDefinedSpeedCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:UserDefinedSpeedCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a UserDefinedSpeedCharacteristicNominal does not identify
            either (1) a UserDefinedSpeedCharacteristicDefinition in the
            document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:UserDefinedTemperatureCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
              /t:UserDefinedTemperatureCharacteristicDefinition[@id = $defnId])
                  + count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a UserDefinedTemperatureCharacteristicNominal does not identify
            either (1) a UserDefinedTemperatureCharacteristicDefinition in
            the document or (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    </xsl:for-each>
    ...
    <xsl:for-each
        select="$charNoms/t:UserDefinedTimeCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:UserDefinedTimeCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a UserDefinedTimeCharacteristicNominal does not identify either
            (1) a UserDefinedTimeCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:UserDefinedUnitCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:UserDefinedUnitCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a UserDefinedUnitCharacteristicNominal does not identify either
            (1) a UserDefinedUnitCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldBevelCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldBevelCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldBevelCharacteristicNominal does not identify either
            (1) a WeldBevelCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldCompoundCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldCompoundCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldCompoundCharacteristicNominal does not identify either
            (1) a WeldCompoundCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldEdgeCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldEdgeCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldEdgeCharacteristicNominal does not identify either
            (1) a WeldEdgeCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldFilletCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldFilletCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldFilletCharacteristicNominal does not identify either
            (1) a WeldFilletCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldFlareBevelCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldFlareBevelCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldFlareBevelCharacteristicNominal does not identify either
            (1) a WeldFlareBevelCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldFlareVCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldFlareVCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldFlareVCharacteristicNominal does not identify either
            (1) a WeldFlareVCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldJCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldJCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldJCharacteristicNominal does not identify either
            (1) a WeldJCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldPlugCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldPlugCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldPlugCharacteristicNominal does not identify either
            (1) a WeldPlugCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldScarfCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldScarfCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldScarfCharacteristicNominal does not identify either
            (1) a WeldScarfCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldSeamCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldSeamCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldSeamCharacteristicNominal does not identify either
            (1) a WeldSeamCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldSlotCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldSlotCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldSlotCharacteristicNominal does not identify either
            (1) a WeldSlotCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldSpotCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldSpotCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldSpotCharacteristicNominal does not identify either
            (1) a WeldSpotCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldSquareCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldSquareCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldSquareCharacteristicNominal does not identify either
            (1) a WeldSquareCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldStudCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldStudCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldStudCharacteristicNominal does not identify either
            (1) a WeldStudCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldSurfacingCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldSurfacingCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldSurfacingCharacteristicNominal does not identify either
            (1) a WeldSurfacingCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldUCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldUCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldUCharacteristicNominal does not identify either
            (1) a WeldUCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WeldVCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WeldVCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WeldVCharacteristicNominal does not identify either
            (1) a WeldVCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$charNoms/t:WidthCharacteristicNominal">
      <xsl:variable name="defnId"
                    select="./t:CharacteristicDefinitionId"/>
      <xsl:if
          test="(count($charDefs
                /t:WidthCharacteristicDefinition[@id = $defnId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $defnId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The CharacteristicDefinitionId (<xsl:value-of select="$defnId"/>)
            of a WidthCharacteristicNominal does not identify either
            (1) a WidthCharacteristicDefinition in the document or
            (2) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
...
<!-- END NOMINAL TO DEFINITION ID INTERNAL CHECKS FOR CHAR'TICS -->
  </xsl:template>
<!-- END PROCESSING INTERNAL CHARACTERISTIC IDS -->

<!-- START PROCESSING INTERNAL FEATURE IDS --> 

  <xsl:template name="processing_feature_ids">
    <xsl:param name="extDocs"/>
    <xsl:variable name="featDefs"
                  select="./t:Features/t:FeatureDefinitions"/>
    <xsl:variable name="featNoms"
                  select="./t:Features/t:FeatureNominals"/>
    <xsl:variable name="featItems"
                  select="./t:Features/t:FeatureItems"/>
    
<!-- Check that each FeatureMeasurement has at most one FeatureItemId. -->
    ...
    <xsl:for-each
      ...
      <xsl:for-each
          select="./t:*">
        <xsl:if
            test="count(./t:FeatureItemId) > 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
            The FeatureMeasurement with id (<xsl:value-of select="./@id"/>)
            has more than one FeatureItemId.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      </xsl:for-each>

<!-- START MEASUREMENT TO ITEM INTERNAL ID CHECKS FOR FEATURES -->

      <xsl:for-each
          select="./t:CircleFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:CircleFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              CircleFeatureMeasurement does not identify either
              (1) a CircleFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CircularArcFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:CircularArcFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              CircularArcFeatureMeasurement does not identify either
              (1) a CircularArcFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ConeFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:ConeFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              ConeFeatureMeasurement does not identify either
              (1) a ConeFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ConicalSegmentFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:ConicalSegmentFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              ConicalSegmentFeatureMeasurement does not identify either
              (1) a ConicalSegmentFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CylinderFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:CylinderFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              CylinderFeatureMeasurement does not identify either
              (1) a CylinderFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CylindricalSegmentFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:CylindricalSegmentFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              CylindricalSegmentFeatureMeasurement does not identify either
              (1) a CylindricalSegmentFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:EdgePointFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:EdgePointFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              EdgePointFeatureMeasurement does not identify either
              (1) a EdgePointFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:EllipseFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:EllipseFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              EllipseFeatureMeasurement does not identify either
              (1) a EllipseFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:EllipticalArcFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:EllipticalArcFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              EllipticalArcFeatureMeasurement does not identify either
              (1) a EllipticalArcFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ElongatedCircleFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:ElongatedCircleFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              ElongatedCircleFeatureMeasurement does not identify either
              (1) a ElongatedCircleFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ElongatedCylinderFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:ElongatedCylinderFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              ElongatedCylinderFeatureMeasurement does not identify either
              (1) a ElongatedCylinderFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ExtrudedCrossSectionFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:ExtrudedCrossSectionFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              ExtrudedCrossSectionFeatureMeasurement does not identify either
              (1) a ExtrudedCrossSectionFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:GroupFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:GroupFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              GroupFeatureMeasurement does not identify either
              (1) a GroupFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:LineFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:LineFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              LineFeatureMeasurement does not identify either
              (1) a LineFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:MarkingFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:MarkingFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              MarkingFeatureMeasurement does not identify either
              (1) a MarkingFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OppositeAngledLinesFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:OppositeAngledLinesFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              OppositeAngledLinesFeatureMeasurement does not identify either
              (1) a OppositeAngledLinesFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OppositeAngledPlanesFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:OppositeAngledPlanesFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              OppositeAngledPlanesFeatureMeasurement does not identify either
              (1) a OppositeAngledPlanesFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OppositeParallelLinesFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:OppositeParallelLinesFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              OppositeParallelLinesFeatureMeasurement does not identify either
              (1) a OppositeParallelLinesFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OppositeParallelPlanesFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:OppositeParallelPlanesFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              OppositeParallelPlanesFeatureMeasurement does not identify either
              (1) a OppositeParallelPlanesFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OtherCurveFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:OtherCurveFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              OtherCurveFeatureMeasurement does not identify either
              (1) a OtherCurveFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OtherNonShapeFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:OtherNonShapeFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              OtherNonShapeFeatureMeasurement does not identify either
              (1) a OtherNonShapeFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OtherShapeFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:OtherShapeFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              OtherShapeFeatureMeasurement does not identify either
              (1) a OtherShapeFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OtherSurfaceFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:OtherSurfaceFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              OtherSurfaceFeatureMeasurement does not identify either
              (1) a OtherSurfaceFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PlaneFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:PlaneFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              PlaneFeatureMeasurement does not identify either
              (1) a PlaneFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PointDefinedCurveFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:PointDefinedCurveFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              PointDefinedCurveFeatureMeasurement does not identify either
              (1) a PointDefinedCurveFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PointDefinedSurfaceFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:PointDefinedSurfaceFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              PointDefinedSurfaceFeatureMeasurement does not identify either
              (1) a PointDefinedSurfaceFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PointFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:PointFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              PointFeatureMeasurement does not identify either
              (1) a PointFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SphereFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:SphereFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              SphereFeatureMeasurement does not identify either
              (1) a SphereFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SphericalSegmentFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:SphericalSegmentFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              SphericalSegmentFeatureMeasurement does not identify either
              (1) a SphericalSegmentFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SurfaceOfRevolutionFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:SurfaceOfRevolutionFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              SurfaceOfRevolutionFeatureMeasurement does not identify either
              (1) a SurfaceOfRevolutionFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ThreadedFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:ThreadedFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              ThreadedFeatureMeasurement does not identify either
              (1) a ThreadedFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ToroidalSegmentFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems
                  /t:ToroidalSegmentFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              ToroidalSegmentFeatureMeasurement does not identify either
              (1) a ToroidalSegmentFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:TorusFeatureMeasurement
                  /t:FeatureItemId">
        <xsl:variable name="itemId" select="."/>
        <xsl:if
            test="(count($featItems/t:TorusFeatureItem[@id = $itemId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $itemId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItemId (<xsl:value-of select="."/>) of a
              TorusFeatureMeasurement does not identify either
              (1) a TorusFeatureItem in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
...
<!-- END MEASUREMENT TO ITEM INTERNAL ID CHECKS FOR FEATURES -->

<!-- Check that each FeatureItem has at most one FeatureNominalId. -->
    ...
    <xsl:for-each
      ...
      <xsl:for-each
          select="./t:*">
        <xsl:if
            test="count(./t:FeatureNominalId) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureItem with id (<xsl:value-of select="./@id"/>)
              does not have exactly one FeatureNominalId.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      </xsl:for-each>

<!-- START ITEM TO NOMINAL INTERNAL ID CHECKS FOR FEATURES -->

      <xsl:for-each
          select="./t:CircleFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:CircleFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              CircleFeatureItem does not identify either
              (1) a CircleFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CircularArcFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:CircularArcFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              CircularArcFeatureItem does not identify either
              (1) a CircularArcFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ConeFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:ConeFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              ConeFeatureItem does not identify either
              (1) a ConeFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ConicalSegmentFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:ConicalSegmentFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              ConicalSegmentFeatureItem does not identify either
              (1) a ConicalSegmentFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CylinderFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:CylinderFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              CylinderFeatureItem does not identify either
              (1) a CylinderFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CylindricalSegmentFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:CylindricalSegmentFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              CylindricalSegmentFeatureItem does not identify either
              (1) a CylindricalSegmentFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:EdgePointFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:EdgePointFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              EdgePointFeatureItem does not identify either
              (1) a EdgePointFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:EllipseFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:EllipseFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              EllipseFeatureItem does not identify either
              (1) a EllipseFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:EllipticalArcFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:EllipticalArcFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              EllipticalArcFeatureItem does not identify either
              (1) a EllipticalArcFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ElongatedCircleFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:ElongatedCircleFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              ElongatedCircleFeatureItem does not identify either
              (1) a ElongatedCircleFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ElongatedCylinderFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:ElongatedCylinderFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              ElongatedCylinderFeatureItem does not identify either
              (1) a ElongatedCylinderFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ExtrudedCrossSectionFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:ExtrudedCrossSectionFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              ExtrudedCrossSectionFeatureItem does not identify either
              (1) a ExtrudedCrossSectionFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:GroupFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:GroupFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              GroupFeatureItem does not identify either
              (1) a GroupFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:LineFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:LineFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              LineFeatureItem does not identify either
              (1) a LineFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:MarkingFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:MarkingFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              MarkingFeatureItem does not identify either
              (1) a MarkingFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OppositeAngledLinesFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:OppositeAngledLinesFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              OppositeAngledLinesFeatureItem does not identify either
              (1) a OppositeAngledLinesFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OppositeAngledPlanesFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:OppositeAngledPlanesFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              OppositeAngledPlanesFeatureItem does not identify either
              (1) a OppositeAngledPlanesFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OppositeParallelLinesFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:OppositeParallelLinesFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              OppositeParallelLinesFeatureItem does not identify either
              (1) a OppositeParallelLinesFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OppositeParallelPlanesFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:OppositeParallelPlanesFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              OppositeParallelPlanesFeatureItem does not identify either
              (1) a OppositeParallelPlanesFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OtherCurveFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:OtherCurveFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              OtherCurveFeatureItem does not identify either
              (1) a OtherCurveFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OtherNonShapeFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:OtherNonShapeFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              OtherNonShapeFeatureItem does not identify either
              (1) a OtherNonShapeFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OtherShapeFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:OtherShapeFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              OtherShapeFeatureItem does not identify either
              (1) a OtherShapeFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OtherSurfaceFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:OtherSurfaceFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              OtherSurfaceFeatureItem does not identify either
              (1) a OtherSurfaceFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PatternFeatureCircleItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:PatternFeatureCircleNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              PatternFeatureCircleItem does not identify either
              (1) a PatternFeatureCircleNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PatternFeatureCircularArcItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:PatternFeatureCircularArcNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              PatternFeatureCircularArcItem does not identify either
              (1) a PatternFeatureCircularArcNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PatternFeatureLinearItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:PatternFeatureLinearNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              PatternFeatureLinearItem does not identify either
              (1) a PatternFeatureLinearNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PatternFeatureParallelogramItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:PatternFeatureParallelogramNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              PatternFeatureParallelogramItem does not identify either
              (1) a PatternFeatureParallelogramNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PlaneFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:PlaneFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              PlaneFeatureItem does not identify either
              (1) a PlaneFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PointDefinedCurveFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:PointDefinedCurveFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              PointDefinedCurveFeatureItem does not identify either
              (1) a PointDefinedCurveFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PointDefinedSurfaceFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:PointDefinedSurfaceFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              PointDefinedSurfaceFeatureItem does not identify either
              (1) a PointDefinedSurfaceFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PointFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:PointFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              PointFeatureItem does not identify either
              (1) a PointFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SphereFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:SphereFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              SphereFeatureItem does not identify either
              (1) a SphereFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SphericalSegmentFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:SphericalSegmentFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              SphericalSegmentFeatureItem does not identify either
              (1) a SphericalSegmentFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SurfaceOfRevolutionFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:SurfaceOfRevolutionFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              SurfaceOfRevolutionFeatureItem does not identify either
              (1) a SurfaceOfRevolutionFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ThreadedFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:ThreadedFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              ThreadedFeatureItem does not identify either
              (1) a ThreadedFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ToroidalSegmentFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms
                  /t:ToroidalSegmentFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              ToroidalSegmentFeatureItem does not identify either
              (1) a ToroidalSegmentFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:TorusFeatureItem
                  /t:FeatureNominalId">
        <xsl:variable name="nomId" select="."/>
        <xsl:if
            test="(count($featNoms/t:TorusFeatureNominal[@id = $nomId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $nomId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominalId (<xsl:value-of select="."/>) of a
              TorusFeatureItem does not identify either
              (1) a TorusFeatureNominal in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
...
<!-- END ITEM TO NOMINAL INTERNAL ID CHECKS FOR FEATURES -->

<!-- Check that each FeatureNominal has at most one FeatureDefinitionId. -->
    ...
    <xsl:for-each
      ...
      <xsl:for-each
          select="./t:*">
        <xsl:if
            test="count(./t:FeatureDefinitionId) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureNominal with id (<xsl:value-of select="./@id"/>)
              does not have exactly one FeatureDefinitionId.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      </xsl:for-each>

<!-- START NOMINAL TO DEFINITION INTERNAL ID CHECKS FOR FEATURES -->

      <xsl:for-each
          select="./t:CircleFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:CircleFeatureDefinition[@id = $defId]) != 1) and
                  (count($extDocs/t:ExternalQIFDocument[@id = $defId]) != 1)">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              CircleFeatureNominal does not identify either
              (1) a CircleFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CircularArcFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:CircularArcFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              CircularArcFeatureNominal does not identify either
              (1) a CircularArcFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ConeFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs/t:ConeFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              ConeFeatureNominal does not identify either
              (1) a ConeFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ConicalSegmentFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:ConicalSegmentFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              ConicalSegmentFeatureNominal does not identify either
              (1) a ConicalSegmentFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CylinderFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs/t:CylinderFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              CylinderFeatureNominal does not identify either
              (1) a CylinderFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:CylindricalSegmentFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:CylindricalSegmentFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              CylindricalSegmentFeatureNominal does not identify either
              (1) a CylindricalSegmentFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:EdgePointFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs/t:EdgePointFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              EdgePointFeatureNominal does not identify either
              (1) a EdgePointFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:EllipseFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs/t:EllipseFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              EllipseFeatureNominal does not identify either
              (1) a EllipseFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:EllipticalArcFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:EllipticalArcFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              EllipticalArcFeatureNominal does not identify either
              (1) a EllipticalArcFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ElongatedCircleFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:ElongatedCircleFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              ElongatedCircleFeatureNominal does not identify either
              (1) a ElongatedCircleFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ElongatedCylinderFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:ElongatedCylinderFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              ElongatedCylinderFeatureNominal does not identify either
              (1) a ElongatedCylinderFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ExtrudedCrossSectionFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:ExtrudedCrossSectionFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              ExtrudedCrossSectionFeatureNominal does not identify either
              (1) a ExtrudedCrossSectionFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:GroupFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs/t:GroupFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              GroupFeatureNominal does not identify either
              (1) a GroupFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:LineFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs/t:LineFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              LineFeatureNominal does not identify either
              (1) a LineFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:MarkingFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:MarkingFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              MarkingFeatureNominal does not identify either
              (1) a MarkingFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OppositeAngledLinesFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:OppositeAngledLinesFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              OppositeAngledLinesFeatureNominal does not identify either
              (1) a OppositeAngledLinesFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OppositeAngledPlanesFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:OppositeAngledPlanesFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              OppositeAngledPlanesFeatureNominal does not identify either
              (1) a OppositeAngledPlanesFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OppositeParallelLinesFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:OppositeParallelLinesFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              OppositeParallelLinesFeatureNominal does not identify either
              (1) a OppositeParallelLinesFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OppositeParallelPlanesFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:OppositeParallelPlanesFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              OppositeParallelPlanesFeatureNominal does not identify either
              (1) a OppositeParallelPlanesFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OtherCurveFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:OtherCurveFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              OtherCurveFeatureNominal does not identify either
              (1) a OtherCurveFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OtherNonShapeFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:OtherNonShapeFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              OtherNonShapeFeatureNominal does not identify either
              (1) a OtherNonShapeFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OtherShapeFeatureFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:OtherShapeFeatureFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              OtherShapeFeatureFeatureNominal does not identify either
              (1) a OtherShapeFeatureFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:OtherSurfaceFeatureFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:OtherSurfaceFeatureFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              OtherSurfaceFeatureFeatureNominal does not identify either
              (1) a OtherSurfaceFeatureFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PatternFeatureCircleNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:PatternFeatureCircleDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              PatternFeatureCircleNominal does not identify either
              (1) a PatternFeatureCircleDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PatternFeatureCircularArcNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:PatternFeatureCircularArcDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              PatternFeatureCircularArcNominal does not identify either
              (1) a PatternFeatureCircularArcDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PatternFeatureLinearNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:PatternFeatureLinearDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              PatternFeatureLinearNominal does not identify either
              (1) a PatternFeatureLinearDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PatternFeatureParallelogramNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:PatternFeatureParallelogramDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              PatternFeatureParallelogramNominal does not identify either
              (1) a PatternFeatureParallelogramDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PlaneFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs/t:PlaneFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              PlaneFeatureNominal does not identify either
              (1) a PlaneFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PointDefinedCurveFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:PointDefinedCurveFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              PointDefinedCurveFeatureNominal does not identify either
              (1) a PointDefinedCurveFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PointDefinedSurfaceFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:PointDefinedSurfaceFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              PointDefinedSurfaceFeatureNominal does not identify either
              (1) a PointDefinedSurfaceFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:PointFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs/t:PointFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              PointFeatureNominal does not identify either
              (1) a PointFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SphereFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs/t:SphereFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              SphereFeatureNominal does not identify either
              (1) a SphereFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SphericalSegmentFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:SphericalSegmentFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              SphericalSegmentFeatureNominal does not identify either
              (1) a SphericalSegmentFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:SurfaceOfRevolutionFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:SurfaceOfRevolutionFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              SurfaceOfRevolutionFeatureNominal does not identify either
              (1) a SurfaceOfRevolutionFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ThreadedFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs/t:ThreadedFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              ThreadedFeatureNominal does not identify either
              (1) a ThreadedFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:ToroidalSegmentFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs
                  /t:ToroidalSegmentFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              ToroidalSegmentFeatureNominal does not identify either
              (1) a ToroidalSegmentFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      ...
      <xsl:for-each
          select="./t:TorusFeatureNominal
                  /t:FeatureDefinitionId">
        <xsl:variable name="defId" select="."/>
        <xsl:if
            test="(count($featDefs/t:TorusFeatureDefinition[@id = $defId]) +
                  count($extDocs/t:ExternalQIFDocument[@id = $defId])) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              The FeatureDefinitionId (<xsl:value-of select="."/>) of a
              TorusFeatureNominal does not identify either
              (1) a TorusFeatureDefinition in the document or
              (2) an external QIF document.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
...
<!-- END NOMINAL TO DEFINITION INTERNAL ID CHECKS FOR FEATURES -->

    </xsl:for-each>
  </xsl:template>
  <!-- END PROCESSING INTERNAL FEATURE IDS -->

  <!-- START PROCESSING INTERNAL PRODUCT IDS -->  
  <xsl:template name="processing_product_ids">
    <xsl:param name="extDocs"/>
    <xsl:variable name="thisDoc" select = "."/>

<!-- ProductFaceParametricSurfaceKeyref
     requires that surfaces whose ids are specified in model faces be
     present in the surface set (SurfaceSet).
-->
    <xsl:variable name="surfaceSet"
                  select="$thisDoc/t:Product/t:GeometrySet/t:SurfaceSet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:TopologySet/t:FaceSet/t:Face
                /t:Surface/t:Id">
      <xsl:variable name="surfaceId" select="."/>
      <xsl:if
          test="(count($surfaceSet/t:*[@id = $surfaceId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $surfaceId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The surface Id (<xsl:value-of select="."/>) of a face
            does not identify either a surface in the document or
...
<!-- ProductLoopParametricCurve12Keyref
     requires that 2D curves whose ids are specified in model loops be
     present in the 2D curve set (Curve12Set).
-->
    <xsl:variable name="curve12Set"
                  select="$thisDoc/t:Product/t:GeometrySet/t:Curve12Set"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:TopologySet/t:LoopSet/t:Loop/t:CoEdges
                /t:CoEdge/t:Curve12/t:Id">
      <xsl:variable name="curveId" select="."/>
      <xsl:if
          test="(count($curve12Set/t:*[@id = $curveId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $curveId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The curve12 Id (<xsl:value-of select="."/>) of a coedge
            does not identify either a curve12 in the document or
            an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
...
<!-- ProductLoopMeshCurveMeshKeyref
     requires that mesh curves whose ids are specified in model loops be
     present in the CurveMeshSet.
-->
    <xsl:variable name="curveMeshSet"
                  select="$thisDoc/t:Product/t:GeometrySet/t:CurveMeshSet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:TopologySet/t:LoopSet/t:LoopMesh
                /t:CoEdgesMesh /t:CoEdgeMesh/t:CurveMesh/t:Id">
      <xsl:variable name="curveMeshId" select="."/>
      <xsl:if
          test="(count($curveMeshSet/t:*[@id = $curveMeshId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $curveMeshId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The curveMesh Id (<xsl:value-of select="."/>) of a coedge mesh
            does not identify either a curveMesh in the document or
            an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
...
<!-- ProductZoneSectionEdgeKeyref
     requires that certain Ids of Edges be the ids of edges in the EdgeSet
     of a Product
-->
    <xsl:variable name="edgeSet"
                  select="$thisDoc/t:Product/t:TopologySet/t:EdgeSet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:ViewSet/t:ZoneSectionSet/t:ZoneSection
                /t:SectionPlanes /t:SectionPlane/t:SectionGroups
                /t:SectionGroup/t:Areas/t:Area /t:Loops/t:Loop/t:Edges/t:Edge
                /t:Id
               | $thisDoc/t:Product/t:ViewSet/t:ZoneSectionSet/t:ZoneSection
                /t:SectionPlanes /t:SectionPlane/t:SectionGroups
                /t:SectionGroup/t:Paths/t:Path /t:Edges/t:Edge/t:Id">
      <xsl:variable name="edgeId" select="."/>
      <xsl:if
          test="(count($edgeSet/t:*[@id = $edgeId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $edgeId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The edge Id (<xsl:value-of select="."/>) of an area or
            path in a section group does not identify either an
            edge in the document or an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    </xsl:for-each>
    
<!-- ProductFaceMeshMeshKeyref
     requires that meshes whose ids are specified in model mesh faces be
     present in the mesh set (MeshSet).
-->
    <xsl:variable name="surfaceMeshSet"
                  select="$thisDoc/t:Product/t:GeometrySet/
                          t:SurfaceMeshSet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:TopologySet/t:FaceSet/t:FaceMesh
                /t:Mesh/t:Id">
      <xsl:variable name="faceMeshId" select="."/>
      <xsl:if
          test="(count($surfaceMeshSet/t:*[@id = $faceMeshId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $faceMeshId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The faceMesh Id (<xsl:value-of select="."/>) of a face mesh
            does not identify either a surfaceMesh in the document or
            an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
...
<!--ProductEdgeCurveKeyref
    requires that 3D curves whose ids are specified in model edges be present
    in the 3D curve set (Curve13Set).
-->
    <xsl:variable name="curve13Set"
                  select="$thisDoc/t:Product/t:GeometrySet/t:Curve13Set"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:TopologySet/t:EdgeSet/t:Edge/t:Curve/t:Id">
      <xsl:variable name="curveId" select="."/>
      <xsl:if
          test="(count($curve13Set/t:*[@id = $curveId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $curveId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The curve Id (<xsl:value-of select="."/>) of an edge
            does not identify either a curve in the document or
            an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
...
<!-- ProductEdgeVertexKeyref
     requires that vertices whose ids are specified in model edges be
     present in the vertex set (VertexSet)
-->
      
    <xsl:variable name="vertexSet"
                  select="$thisDoc/t:Product/t:TopologySet/t:VertexSet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:TopologySet/t:EdgeSet/t:Edge
                /t:VertexBeg/t:Id">
      <xsl:variable name="vertexBegId" select="."/>
      <xsl:if
          test="(count($vertexSet/t:Vertex[@id = $vertexBegId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $vertexBegId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The VertexBeg Id (<xsl:value-of select="."/>) of an edge
            does not identify either a vertex in the document or
            an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$thisDoc/t:Product/t:TopologySet/t:EdgeSet/t:Edge
                /t:VertexEnd/t:Id">
      <xsl:variable name="vertexEndId" select="."/>
      <xsl:if
          test="(count($vertexSet/t:Vertex[@id = $vertexEndId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $vertexEndId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The VertexEnd Id (<xsl:value-of select="."/>) of an edge
            does not identify either a vertex in the document or
            an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:for-each
        select="$thisDoc/t:Product/t:ViewSet/t:ZoneSectionSet/t:ZoneSection
                /t:SectionPlanes/t:SectionPlane/t:SectionGroups
                /t:SectionGroup/t:Vertices/t:Id">
      <xsl:variable name="vertexId" select="."/>
      <xsl:if
          test="(count($vertexSet/t:Vertex[@id = $vertexId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $vertexId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            A vertex Id (<xsl:value-of select="."/>) in the ViewSet
            does not identify either a vertex in the document or
            an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
...
<!-- ProductVertexPointKeyref
     requires that points whose ids are specified in model vertices be
     present in the point set (PointSet)
-->
    <xsl:variable name="pointSet"
                  select="$thisDoc/t:Product/t:GeometrySet/t:PointSet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:TopologySet/t:VertexSet/t:Vertex
                /t:Point/t:Id">
      <xsl:variable name="pointId" select="."/>
      <xsl:if
          test="(count($pointSet/t:Point[@id = $pointId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $pointId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The point Id (<xsl:value-of select="."/>) of a vertex
            does not identify either a point in the document or
            an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
...
<!-- ProductBodiesIdKeyref
     requires that an id in the Product element that should be the id
     of a body is the id of a body in the bodySet of the Product
-->
    <xsl:variable name="bodySet"
                  select="$thisDoc/t:Product/t:TopologySet/t:BodySet"/>
    <xsl:variable name="bodyIds"
                  select="$thisDoc/t:Product/t:ViewSet/t:SavedViewSet
                           /t:SavedView/t:BodyIds
                          | $thisDoc/t:Product/t:ViewSet
                           /t:SimplifiedRepresentationSet
                           /t:SimplifiedRepresentation
                           /t:SimplifiedRepresentationGroups
                           /t:SimplifiedRepresentationGroup/t:BodyIds
                          | $thisDoc/t:Product/t:ViewSet/t:ExplodedViewSet
                           /t:ExplodedView /t:MoveGroups/t:MoveGroup/t:BodyIds
                          | $thisDoc/t:Product/t:ViewSet/t:DisplayStyleSet
                           /t:DisplayStyle /t:DisplayStyleGroups
                           /t:DisplayStyleGroup/t:BodyIds
                          | $thisDoc/t:Product/t:PartSet/t:Part/t:BodyIds
                          | $thisDoc/t:Product/t:AssemblySet/t:Assembly
                           /t:BodyIds"/>
    <xsl:variable name="sectionGroup"
                  select="$thisDoc/t:Product/t:ViewSet/t:ZoneSectionSet
                          /t:ZoneSection/t:SectionPlanes/t:SectionPlane
                          /t:SectionGroups/t:SectionGroup"/>
    <xsl:for-each
        select="$bodyIds/t:Id">
      <xsl:variable name="bodyId" select="."/>
      <xsl:if
          test="(count($bodySet/t:Body[@id = $bodyId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $bodyId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            An id (<xsl:value-of select="."/>) that should be the id of
            a body does not identify either a body in the document or
            an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
...
<!-- ProductZoneSectionIdKeyref
     requires that clipping planes whose ids are specified in model saved
     views be present in the clipping plane set (PlaneClipping)

-->
    <xsl:variable name="zoneSectionSet"
                  select="$thisDoc/t:Product/t:ViewSet/t:ZoneSectionSet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:ViewSet/t:SavedViewSet/t:SavedView
                /t:ZoneSectionId">
      <xsl:variable name="zoneSectionId" select="."/>
      <xsl:if
          test="(count($zoneSectionSet/t:ZoneSection[@id = $zoneSectionId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $zoneSectionId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The ZoneSectionId (<xsl:value-of select="."/>) of a saved view
            does not identify either a zone section in the document or
            an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
...
<!-- ProductCoordinateSystemsIdKeyref
     requires that coordinate systems whose ids are specified in
     CoordinateSystems be present in the coordinate system set
     (CoordinateSystemSet)
-->
    
    <xsl:variable name="coordSysSet"
                  select="$thisDoc/t:Product/t:CoordinateSystemSet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:PartSet/t:Part/t:CoordinateSystemIds/t:Id">
      <xsl:variable name="coordSysId" select="."/>
      <xsl:if
          test="(count($coordSysSet/t:CoordinateSystem[@id = $coordSysId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $coordSysId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The coordinate system Id (<xsl:value-of select="."/>) of a
            part does not identify either a coordinate system in the
            document or an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    </xsl:for-each>
    <xsl:for-each
        select="$thisDoc/t:Product/t:AssemblySet/t:Assembly
                /t:CoordinateSystemIds/t:Id">
      <xsl:variable name="coordSysId" select="."/>
      <xsl:if
          test="(count($coordSysSet/t:CoordinateSystem[@id = $coordSysId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $coordSysId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The coordinate system Id (<xsl:value-of select="."/>) of an
            assembly does not identify either a coordinate system in the
            document or an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
...
<!-- ProductAuxiliaryIdKeyref
     requires that auxiliary objects whose ids are specified in Auxiliaries
     be present in the auxiliary object set (AuxiliarySet)
-->
    <xsl:variable name="auxiliarySet"
                  select="$thisDoc/t:Product/t:AuxiliarySet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:PartSet/t:Part/t:AuxiliaryIds/t:Id">
      <xsl:variable name="auxiliaryId" select="."/>
      <xsl:if
          test="(count($auxiliarySet/t:*[@id = $auxiliaryId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $auxiliaryId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The auxiliary Id (<xsl:value-of select="."/>) of a
            part does not identify either an item of auxiliary geometry
            in the document or an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    </xsl:for-each>
    <xsl:for-each
        select="$thisDoc/t:Product/t:AssemblySet/t:Assembly/t:AuxiliaryIds/t:Id">
      <xsl:variable name="auxiliaryId" select="."/>
      <xsl:if
          test="(count($auxiliarySet/t:*[@id = $auxiliaryId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $auxiliaryId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            The auxiliary Id (<xsl:value-of select="."/>) of an
            assembly does not identify either an item of auxiliary geometry
            in the document or an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
...
<!-- ProductViewIdKeyref
     requires that view objects whose ids are specified in Views be present
     in the view object set (ViewSet)
-->
    <xsl:variable name="viewSet"
                  select="$thisDoc/t:Product/t:ViewSet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:PartSet/t:Part/t:ViewIds/t:Id">
      <xsl:variable name="viewId" select="."/>
      <xsl:if
          test="(count($viewSet/t:*/t:*[@id = $viewId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $viewId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            A view Id (<xsl:value-of select="."/>) of a part does not
            identify either an object in the view set of the
            document or an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$thisDoc/t:Product/t:AssemblySet/t:Assembly/t:ViewIds/t:Id">
      <xsl:variable name="viewId" select="."/>
      <xsl:if
          test="(count($viewSet/t:*/t:*[@id = $viewId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $viewId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            A view Id (<xsl:value-of select="."/>) of an assembly does
            not identify either an object in the view set of the
            document or an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
...
<!-- ProductPointCloudIdKeyref
     requires that point clouds whose ids are in PointCloudsIds of parts
     or assemblies be present in the point cloud set (PointCloudSet)
-->

    <xsl:variable name="pointCloudSet"
                  select="$thisDoc/t:Product/t:TopologySet/t:PointCloudSet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:PartSet/t:Part/t:PointCloudIds/t:Id">
      <xsl:variable name="pointCloudId" select="."/>
      <xsl:if
          test="(count($pointCloudSet/t:PointCloud[@id = $pointCloudId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $pointCloudId]))
                != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            A PointCloud Id (<xsl:value-of select="."/>) of a part
            does not identify either a point cloud in the point cloud
            set of the document or an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    </xsl:for-each>
    <xsl:for-each
      ...
      <xsl:if
        ...
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            A PointCloud Id (<xsl:value-of select="."/>) of an assembly
            does not identify either a point cloud in the point cloud
            set of the document or an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
...
<!-- ProductPartNoteKeyref
     requires that an Id in the the PartNoteIds of a part or assembly be
     the id of either (1) a PartNote in the PartNoteSet of the Product or
     (2) an external QIF file.
-->
    <xsl:variable name="partNoteSet"
                  select="$thisDoc/t:Product/t:PartNoteSet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:PartSet/t:Part/t:PartNoteIds/t:Id">
      <xsl:variable name="partNoteId" select="."/>
      <xsl:if
          test="(count($partNoteSet/t:PartNote[@id = $partNoteId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $partNoteId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            A PartNote Id (<xsl:value-of select="."/>) of a part
            does not identify either a part note in the part note
            set of the document or an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    ...
    <xsl:for-each
        select="$thisDoc/t:Product/t:AssemblySet/t:Assembly/t:PartNoteIds/t:Id">
      <xsl:variable name="partNoteId" select="."/>
      <xsl:if
          test="(count($partNoteSet/t:PartNote[@id = $partNoteId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $partNoteId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            A PartNote Id (<xsl:value-of select="."/>) of an assembly
            does not identify either a part note in the part note
            set of the document or an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:for-each
        select="$thisDoc/t:Product/t:PartNoteSet/t:PartNote/t:PartNoteIds/t:Id">
      <xsl:variable name="partNoteId" select="."/>
      <xsl:if
          test="(count($partNoteSet/t:PartNote[@id = $partNoteId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $partNoteId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            A PartNote Id (<xsl:value-of select="."/>) of a part note
            does not identify either a part note in the part note
            set of the document or an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
...
<!-- NoteKeyref
     requires that an Id in the the NoteIds of a part or assembly be
     the id of one of:
     (1) a Note in the NoteSet of the Product
     (2) a NoteFlag in the NoteFlagSet of the Product
     (3) an external QIF file
-->
    <xsl:variable name="noteSet" select="$thisDoc/t:Product/t:NoteSet"/>
    <xsl:variable name="noteFlagSet" select="$thisDoc/t:Product/t:NoteFlagSet"/>
    <xsl:for-each
        select="$thisDoc/t:Product/t:PartSet/t:Part/t:NoteIds/t:Id">
      <xsl:variable name="noteId" select="."/>
      <xsl:if
          test="(count($noteSet/t:Note[@id = $noteId]) +
                count($noteFlagSet/t:Note[@id = $noteId]) +
                count($extDocs/t:ExternalQIFDocument[@id = $noteId])) != 1">
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            A Note Id (<xsl:value-of select="."/>) of a part does not
            identify any of (1) a note in the note set of the document
            (2) a note flag in the note flag set of the document
            (3) an external QIF document.
          </xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    </xsl:for-each>
    <xsl:for-each
        select="$thisDoc/t:Product/t:AssemblySet/t:Assembly/t:NoteIds/t:Id">
      <xsl:variable name="noteId" select="."/>
      <xsl:if
          test="(count($noteSet/t:Note[@id = $noteId]) +
                count($noteFlagSet/t:Note[@id = $noteId]) +
        ...
        <xsl:call-template name="error_node">
          <xsl:with-param name="report">
            A Note Id (<xsl:value-of select="."/>) of an assembly does not
            identify any of (1) a note in the note set of the document
            (2) a note flag in the note flag set of the document
            (3) an external QIF document.
          </xsl:with-param>
...
<!-- FoldersPartFolderIdKeyref
     requires that for each part, folders whose ids are specified in
     a FolderIds be present in the FoldersPart
-->
    <xsl:for-each select="$thisDoc/t:Product/t:PartSet/t:Part">
      <xsl:variable name="aPart" select="."/>
      <xsl:variable name="partFolders"
                    select="$aPart/t:FoldersPart"/>
      <xsl:for-each
          select="$partFolders/t:FolderPart/t:FolderIds/t:Id">
        <xsl:variable name="folderId"
                      select="."/>
        <xsl:if
            test="count($partFolders/t:FolderPart[@id = $folderId]) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              An Id (<xsl:value-of select="."/>) in the FolderIds of the
              part with id (<xsl:value-of select="$aPart/@id"/>) does not
              identify another part folder for the same part.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      </xsl:for-each>
...
<!-- FoldersAssemblyFolderIdKeyref
     requires that for each assembly, folders whose ids are specified in
     a FolderIds be present in the FoldersAssembly
-->
    <xsl:for-each select="$thisDoc/t:Product/t:AssemblySet/t:Assembly">
      <xsl:variable name="anAssembly" select="."/>
      <xsl:variable name="assemblyFolders"
                    select="$anAssembly/t:FoldersAssembly"/>
      <xsl:for-each
          select="$assemblyFolders/t:FolderAssembly/t:FolderIds/t:Id">
        <xsl:variable name="folderId"
                      select="."/>
        <xsl:if
            test="count($assemblyFolders/t:FolderAssembly[@id = $folderId]) != 1">
          <xsl:call-template name="error_node">
            <xsl:with-param name="report">
              An Id (<xsl:value-of select="."/>) in the FolderIds of the
              assembly with id (<xsl:value-of select="$anAssembly/@id"/>) does
              not identify another assembly folder for the same assembly.
            </xsl:with-param>
          </xsl:call-template>
        </xsl:if>
      </xsl:for-each>
...
<!-- END PROCESSING THIS DOCUMENT -->
  
<!-- START PROCESSING LINKED DOCUMENTS -->
  <xsl:template name="processing_linked_documents">
    <xsl:param name="caller"/>
    <xsl:for-each
    select="t:ExternalReferences/t:ExternalQIFReferences/t:ExternalQIFDocument">
...
<!-- START EXTERNAL ID CHECKS FOR CHAR'TICS -->
<!-- START CALLING MEASUREMENT TO ITEM ID EXTERNAL CHECKS FOR CHAR'TICS -->
          <xsl:call-template
...
<!-- END CALLING MEASUREMENT TO ITEM ID EXTERNAL CHECKS FOR CHARACTERISTICS -->

<!-- START CALLING ITEM TO NOMINAL ID EXTERNAL CHECKS FOR CHARACTERISTICS -->
          <xsl:call-template
...
<!-- END CALLING ITEM TO NOMINAL ID EXTERNAL CHECKS FOR CHARACTERISTICS -->
 
<!-- START CALLING NOMINAL TO DEFINITION ID EXTERNAL CHECKS FOR CHAR'TICS -->
              <xsl:call-template
...
<!-- END CALLING NOMINAL TO DEFINITION ID EXTERNAL CHECKS FOR CHAR'TICS -->
<!-- END CALLING EXTERNAL ID CHECKS FOR CHAR'TICS -->

<!-- START CALLING EXTERNAL ID CHECKS FOR FEATURES -->
<!-- START CALLING MEASUREMENT TO ITEM EXTERNAL ID CHECKS FOR FEATURES -->
              <xsl:call-template
...
<!-- END CALLING MEASUREMENT TO ITEM EXTERNAL ID CHECKS FOR FEATURES -->

<!-- START CALLING ITEM TO NOMINAL EXTERNAL ID CHECKS FOR FEATURES -->
              <xsl:call-template
...
<!-- END CALLING ITEM TO NOMINAL EXTERNAL ID CHECKS FOR FEATURES -->

<!-- START CALLING NOMINAL TO DEFINITION EXTERNAL ID CHECKS FOR FEATURES -->
              <xsl:call-template
...
<!-- END CALLING NOMINAL TO DEFINITION EXTERNAL ID CHECKS FOR FEATURES -->
<!-- END CALLING EXTERNAL ID CHECKS FOR FEATURES -->
...
<!-- START DEFINING  MEASUREMENT TO ITEM EXTERNAL ID CHECKS FOR CHARACTERISTICS
...
<!-- END MEASUREMENT TO ITEM EXTERNAL ID CHECKS FOR CHARACTERISTICS -->
 
<!-- START DEFINING ITEM TO NOMINAL EXTERNAL ID CHECKS FOR CHARACTERISTICS
...
<!-- END ITEM TO NOMINAL EXTERNAL ID CHECKS FOR CHARACTERISTICS -->

<!-- START DEFINING NOMINAL TO DEFINITION EXTERNAL ID CHECKS FOR CHARACTERISTICS
...
<!-- END NOMINAL TO DEFINITION EXTERNAL ID CHECKS FOR CHARACTERISTICS -->
<!-- START DEFINING ID CHECKS FOR FEATURES -->
<!-- START DEFINING MEASUREMENT TO ITEM EXTERNAL ID CHECKS FOR FEATURES
...
<!-- END MEASUREMENT TO ITEM EXTERNAL ID CHECKS FOR FEATURES -->

<!-- START DEFINING ITEM TO NOMINAL EXTERNAL ID CHECKS FOR FEATURES
...
<!-- END ITEM TO NOMINAL EXTERNAL ID CHECKS FOR FEATURES -->

<!-- START DEFINING NOMINAL TO DEFINITION EXTERNAL ID CHECKS FOR FEATURES
...
<!-- END NOMINAL TO DEFINITION EXTERNAL ID CHECKS FOR FEATURES -->
<!-- END PROCESSING LINKED DOCUMENTS -->
